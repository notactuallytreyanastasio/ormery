...
// core module removed — HTML autoescaping not needed for SQL query builder
...
impl Query {
    ...
    pub fn to_sql(& self) -> crate::sql::SqlFragment {
        return to_sql_query(self.0.read().unwrap().schema.clone(), self.0.read().unwrap().select_fields.clone(), temper_core::ListedTrait::to_list( & self.0.read().unwrap().where_clauses), temper_core::ListedTrait::to_list( & self.0.read().unwrap().order_by_clauses), self.0.read().unwrap().limit_value, self.0.read().unwrap().offset_value);
    }
    pub fn schema(& self) -> Schema {
        return self.0.read().unwrap().schema.clone();
    }
...
impl Token {
    ...
    // to_html removed — html module not compiled for SQL-only usage
...
// highlight_word, highlight_line, highlight_source, highlight_block removed
// — html module not compiled for SQL-only usage
#[cfg(test)]
mod tests {
    #[test]
